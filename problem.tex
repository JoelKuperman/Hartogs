...
\begin{definition}
	...
	\begin{enumerate}[(i)]
		...
		\item If $\langle A,< \rangle$ is an infinite $\estf$ cardinal, we define its length as $\mathcal{C}(A,<) = \mathcal{C}(A) = \{x\in A: (x{\downarrow},<_x) \text{ is an infinite $\estf$ cardinal}\}$. We give $\mathcal{C}(A)$ the inherited order from $A$.
	\end{enumerate}
\end{definition}
...
\begin{lemma}
	\label{weak_aleph}
	Let $A$ and $B$ be two infinite $\estf$ cardinals, then $A <_c B$ iff $\mathcal{C}(A) <_o \mathcal{C}(B)$ and $A =_c B$ iff $\mathcal{C}(A) =_o \mathcal{C}(B)$.
\end{lemma}
