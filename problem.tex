...
The previous result relies heavily on Choice, in fact, given any partially ordered set $\langle P,< \rangle$ it is consistent with ZF for there to be a family of sets $\mathcal{F}$ such that $\langle \mathcal{F},<_c\rangle$ is isomorphic to $\langle P,< \rangle$ (Jech, Theorem 11.1 \cite{Jech_AC}).
...
Without Replacement or Choice it is consistent for several common definitions of infinite set to not be equivalent between each other, (See Dedekind finite sets, amorphous sets, etc). 
...
\end{lemma}
\begin{proof}
	Lets denote by $a_0$ and $b_0$ the $<$-minimal element of $\langle A, <\rangle$ and the $\sqsubset$-minimal element of $\langle B, \sqsubset \rangle$ respectively. Now, we define $f$ recursively by $f(a_0)=b_0$, $f(x+1)=f(x)+1$. We claim that $f$ is an order isomorphism: 
	\begin{itemize}
