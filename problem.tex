...
%\end{question}
In our context we recall that it is an open problem whether the following $\ZFC$ theorem,
\begin{quote}
  \emph{Nondecreasing Sequence principle (NDS)}:
  There is no sequence $\{X_n\}_{n\in\omega}$ such that for all $n\in\omega$ $X_{n+1} <_c X_n$.
\end{quote}
is equivalente to full $\AC$ over $\ZF$. Its absence it is consistent for it to fail in any way: given a partially ordered set $\langle P,< \rangle$ it is consistent with ZF for there to be a family of sets $\mathcal{F}$ such that $\langle \mathcal{F},<_c\rangle$ is isomorphic to $\langle P,< \rangle$ (Jech, Theorem 11.1 \cite{Jech_AC}).
...
Assuming there is a Peano system, $\estf$ proves that one can define functions recursively (\cite[Theorem 5.6]{Moscho}). From there it is straightforward to define addition in a given Peano system as $n+0 = n$ and $n+S(m) = S(n+m)$ for each $n,m$.
...
\begin{lemma}[$\esti$]
	...
	\begin{enumerate}[(1)]
		...
		\item Let $(\mathbb{N}, 0, S)$ be a Peano System. Define $n<m$ iff $\exists k\in\mathbb{N}{\setminus} \{0\}$ such that $n+k = m$. Then $\langle \mathbb{N},< \rangle$ is a well ordering and it has type $\omega$.
	\end{enumerate}
\end{lemma}
...
%\end{lemma}
\begin{proof}
	Let us start by proving \emph{(1)}. First we note that ``$y$ is an immediate successor of $x$'' can be written in first order logic (in the language of set theory if we need to) as $x<y \land \forall z\in A (z < y \implies (z=x \lor z < x))$, which we will abbreviate as $\text{Succ}(x,y)$. Having done this, we can use Comprehension on the set $A\times A$: we simply take $S_A$ as $\{(x,y)\in A\times A: \text{Succ}(x,y)\}$. Let us now show that $(A,a_0,S_A)$ is indeed a Peano system. Properties (1) through (4) of Definition \ref{peano} are an immediate consequence of the hypothesis on $\langle A,< \rangle$. Let $X\subseteq A$ be a set such that 
	\[
	...
	To show \emph{(2)}, we first note that $\langle \mathbb{N},< \rangle$ is a well ordering by \cite[Theorem 5.23]{Moscho}. The fact that it also has type $\omega$ is trivial from the definition of $<$.
\end{proof}
...
\begin{theorem}[$\esti$]
	\label{hartogs_set}
	Let $(\mathbb{N},0,S)$ be a Peano System, then there is no sequence $\{X_n\}_{n\in \mathbb{N}}$ such that for all $n\in A$ ($\mathcal{P}(X_{n+1}) <_c X_n$).
\end{theorem}
...
\begin{proof}
	...
	\noindent Thus, for all $X\in\ran(\mathcal{S})$, we have $\aleph(X)\in\mathcal{P}(\mathcal{P}(\WO(\union \ran(\mathcal{S}))))$. Hence, $\esti$ proves the existence of the sequence $\{\aleph(X_n)\}_{n\in \mathbb{N}}$, which would of course be trivial using Replacement.
	
	It is straightforward to show that for each $n\in \mathbb{N}$, 
...
%	\end{align*}	
	...
	\noindent Let us now show that $\aleph(X_{n+4}) <_c \aleph(X_n)$ for all $n\in \mathbb{N}$. Well ordered sets are pairwise comparable with the relation $\leq_c$, in particular: $\aleph(X_{n+4}) <_c \aleph(X_n)$ or $\aleph(X_n) \leq_c \aleph(X_{n+4})$. The latter statement cannot hold, since it would imply $\aleph(X_n) <_c X_n$, which contradicts Lemma \ref{aleph2}.
	
	Finally, $\{\aleph(X_n)\}_{n\in \mathbb{N}}$
