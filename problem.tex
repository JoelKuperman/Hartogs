...
The following two lemmas show that the length of EST cardinals has the properties we would expect when working with cardinals in stronger theories.

\begin{lemma}
	\label{weak_aleph}
	Let $A$ and $B$ be two infinite $\estf$ cardinals, then $A <_c B$ iff $\mathcal{L}(A) <_o \mathcal{L}(B)$ and $A =_c B$ iff $\mathcal{L}(A) =_o \mathcal{L}(B)$.
\end{lemma}
