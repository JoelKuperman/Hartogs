...
\begin{notation}
	Let us say that an order has \emph{type $\omega$} iff it has a first element, every element has an immediate successor, and every element (except the first one) is someone's immediate successor. Note that $\langle \omega,\in \rangle$ has type $\omega$ in models where it exists.
\end{notation}
...
\begin{lemma}[$\esti$]
	Let $\langle A,< \rangle$ and $\langle B,\sqsubset \rangle$ be two well orders such that of type $\omega$, then $\langle A,< \rangle$ and $\langle B,\sqsubset \rangle$ are order isomorphic.
\end{lemma}
\begin{proof}
